...
export const usePostStore = defineStore('post', () => {
  ...
  async function createPost(data: CreatePostData, authorId: number) {
    loading.value = true;
    try {
      const response = await api.post(`/posts/?author_id=${authorId}`, data);
  ...
  async function fetchTags(params?: { skip?: number; limit?: number }) {
    loading.value = true;
    try {
      const response = await api.get('/posts/tags/', { params });
  ...
  async function fetchTag(id: number): Promise<TagWithPosts> {
    loading.value = true;
    try {
      const response = await api.get<TagWithPosts>(`/posts/tags/${id}`);
  ...
  async function createTag(data: CreateTagData) {
    loading.value = true;
    try {
      const response = await api.post('/posts/tags/', data);
  ...
  async function updateTag(id: number, data: UpdateTagData) {
    loading.value = true;
    try {
      const response = await api.patch(`/posts/tags/${id}`, data);
